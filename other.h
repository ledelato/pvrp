#define USE_CONSISTENT_ROUTES 1
#define DO_NOT_USE_CONSISTENT_ROUTES 0
  ...
  for (r = 0; r < d_total; r = r + 1) /*each day*/
  {
    for (k = 0; k < v_total; k = k + 1) /*for each vehicle*/
    {
      if (!route[r][k].nodes.empty()) /*if the route is not empty*/
      {
        ...
        } //otherwise, over does not change
      }
    }
  }
...
double cal_cost(const vector<vector<Path> >& route,
...
vector<vector<list<int> > > check_all(const vector<vector<list<int> > >& route,
    int d_total, int v_total) {
  vector < vector<list<int> >
      > route_new(d_total + 3,
          vector < list<int> > (v_total + 3, list<int>(4, -100)));
  route_new = route;
...
/**
 * Initialize vehicle routes for each day.
 */
...
vector<vector<Path> > route_initial(const vector<vector<int> >& assign) {
  ...
  for (i = 0; i < d_total; i++) /*on each day*/
  {
    for (j = 0; j < n_total; j++) /*for each node*/
    {
      if (assign[j][i] != 0) /*if the node is visited*/
      {
        ...
        if (route[i][vtemp - 1].nodes.empty()) /*if j is the first node in the route*/
        {
          ...
          route[i][vtemp - 1].nodes.push_back(j + 1);
      }
    }
  }
...
vector<vector<int> > assign_initial(int type) {
  ...
  switch (type) {
  case USE_CONSISTENT_ROUTES: //remain consistency
    for (i = 0; i < n_total; i = i + 1) /*for each node*/
    {
    ...
    break;
  case DO_NOT_USE_CONSISTENT_ROUTES:
    ...
    break;
  default:
    throw "invalid parameter for route consistency, only options are 1 and 0";
  }
...
bool isnew(const vector<vector<int> > &assigntemp,
    const vector<vector<int> >& assign, int node) {
  int i;
