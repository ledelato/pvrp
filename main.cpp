  ...
  for (int i=0 ; i<d_total ; i++ ) {
    ...
    for (int j=0 ; j<v_total ; j++ ) {
      ...
      for (int k=0 ; k<=n_total ; k++ ) {
        ...
        for (it = route[i][j].nodes.begin() ; it != route[i][j].nodes.end() ; it++) {
          if ( *it != k && *it != -100 ) 
            nodes.push_back(*it);
        }
...
void fix_neighbours(solution_change c) {
  ...
  for (int i=0 ; i<=n_total ; i++ ) {
    for (int j=0 ; j<(signed)neighbours[c.day][c.old_route][i].size() ; j++ )
      if (neighbours[c.day][c.old_route][i][j] == c.moved_node) {
        neighbours[c.day][c.old_route][i][j] = neighbours[c.day][c.old_route][i][neighbours[c.day][c.old_route][i].size()-1];
        neighbours[c.day][c.old_route][i].pop_back();
        root = i;
        sort(neighbours[c.day][c.old_route][i].begin(), neighbours[c.day][c.old_route][i].end(), cmp);
      }
  }
  ...
  for (int i=0 ; i<=n_total ; i++ ) {
    if (i != c.moved_node) {
      ...
      root = i;
      sort(neighbours[c.day][c.new_route][i].begin(), neighbours[c.day][c.new_route][i].end(), cmp);
    }
  }
    ...
    for (int r=0 ; r<v_total ; r++ ) {
      ...
      for (int i=0 ; i<n_total ; i++ ) {
        ...
        for (int j=0 ; j<p && j<neighbours[d][r][i].size() ; j++ ) {
          cout << " (" << neighbours[d][r][i][j] << "," << new_d[i][neighbours[d][r][i][j]] << ") ";
        }
        cout << endl;
      }
    }
...
void calculate_visit_time(vector <vector <Path> > &route, vector<vector<int> > &visit_time) {
...
void calculate_region_time(vector <vector <Path> > &route, vector<vector<int> > &region_time){
...
int main ()
{
  ...
  //int infcount = 0; //infeasibility route count
  ...
  for(int i = 0; i<= d_total; i++){
    con_weight[i] = alpha * con_weight[i];
    cout<<con_weight[i]<<" ";
  }
  cout<<endl;
  ...
  /*if(flag_read==1)
  {
    printf("Error: can't read in data.\n");
    return 1;
  } 
  else
  {
    printf("Data readed in successfully.\n");
  }*/
  ...
  vector<vector<vector<int> > > rho(n_total+4, vector<vector<int> >(d_total+4,vector<int>(v_total,0)));               /*rho matrix*/
  vector<vector<vector<int> > > tau(n_total+4, vector<vector<int> >(d_total+4,vector<int>(v_total,0)));               /*tau matrix*/
  vector<vector<vector<double> > > aspiration(n_total+4, vector<vector<double> >(d_total+4,vector<double>(v_total+4, DBL_MAX)));      /*sigma matrix*/
  ...
  double objlast = DBL_MAX;;          /*record objective value of last iteration*/
  ...
  if(initfeas)    /*if initial solution is feasible*/
  {
    ...
    for(i =0; i<n_total; i = i+1)   /*for every node*/
    {
      ...
      {
        vtemp = assign[i][j];
        if(vtemp!=0)  aspiration[i][j][vtemp-1] = objbest; 
      }
    }
  }
  ...
  for (lam = 1;lam < i_total; lam = lam+1)
  {
    ...
    else
    {
            ...
            {   
              ...
              (isnear(route[k], (i+1), neighbor, p, q)): route j contains q nearest neighbor of current node*/
              {
                ...
                {                                                                                           
                  ...
                  n = 0;
                  for(it = route[r][j].nodes.begin(); it!=route[r][j].nodes.end(); it++)
                  {
                    myints[n] = *it;
                    n = n+1;
                  }
                  addroute = geni_insert_cached(route[r][j], (i+1), r, j, p, new_d, myints,n);
                  ...
                  {                                           
                    ...
                    n = 0;
                    for(it = route[r][vtemp-1].nodes.begin(); it!=route[r][vtemp-1].nodes.end(); ++it)
                    {
                    ...
                    delroute = geni_remove_cached(i+1, vtemp-1, r, route[r][vtemp-1], p, new_d, myints, n);

                    delete []myints;
                    
                    totaltemp = oldroute_total + addroute.cost + delroute.cost - routetemp[r][j].cost - routetemp[r][vtemp-1].cost;

                    overcap_temp = overcap_old - ((routetemp[r][j].demand > cap)? (routetemp[r][j].demand - cap) : 0)
                      - ((routetemp[r][vtemp-1].demand > cap)? (routetemp[r][vtemp-1].demand - cap): 0)
                    ...
                    consist_temp = total_consist - visit_time[i][vtemp-1]*con_weight[visit_time[i][vtemp-1]]
                                 - visit_time[i][j]*con_weight[visit_time[i][j]]
                                 + (visit_time[i][vtemp-1]-1)*con_weight[visit_time[i][vtemp-1]-1]
                                 + (visit_time[i][j]+1)*con_weight[visit_time[i][j]+1];

                    region_temp = total_region 
                          +((region_freq[r][vtemp-1][reg[i]]==1)? 
                          ((region_time[reg[i]][vtemp-1]-1)*reg_weight[region_time[reg[i]][vtemp-1]-1]
                            - region_time[reg[i]][vtemp-1]*reg_weight[region_time[reg[i]][vtemp-1]]) : 0)
                          +((region_freq[r][j][reg[i]]==0)? ((region_time[reg[i]][j]+1)*reg_weight[region_time[reg[i]][j]+1]
                                - region_time[reg[i]][j]*reg_weight[region_time[reg[i]][j]]) : 0);

                    //assert(abs(consist_temp-region_temp)<EP);

                    objtemp = totaltemp + consist_temp + region_temp + cap_factor*overcap_temp;
                    if (DEBUG)
                      cout << "Obj cost : " << objtemp << " " << cal_obj((totaltemp+consist_temp), routetemp) << endl;
                ...
                if(objtemp>=objective)
                {
                  ...
                  /*add penalty cost*/
                  objtemp = objtemp
                    +div_factor*(sqrt(double(n_total*v_total*d_total)))*freq*(oldroute_total+total_consist+total_region)/lam;
                
                }
                else
                {
                  objtemp = objtemp;    /*no penalty*/
                }
                                    
                if((tau[i][r][j]<lam) || (tau[i][r][j]>=lam && isfeasible(assigntemp,cap, de_rank, d_total, v_total, n_total) && aspiration[i][r][j]>totaltemp ))
                {
      ...
      if (1||DEBUG)
      {
        cout << "Change " << change.moved_node << " " << change.old_route << " " << change.new_route << " " << change.day << endl;
      //  system("pause");
      }

      //change consistency
      total_consist = total_consist 
              - visit_time[change.moved_node-1][change.old_route]*con_weight[visit_time[change.moved_node-1][change.old_route]]
              - visit_time[change.moved_node-1][change.new_route]*con_weight[visit_time[change.moved_node-1][change.new_route]]
              +(visit_time[change.moved_node-1][change.old_route]-1)*con_weight[visit_time[change.moved_node-1][change.old_route]-1]
              +(visit_time[change.moved_node-1][change.new_route]+1)*con_weight[visit_time[change.moved_node-1][change.new_route]+1];

      
      visit_time[change.moved_node-1][change.old_route] = visit_time[change.moved_node-1][change.old_route] - 1;
      visit_time[change.moved_node-1][change.new_route] = visit_time[change.moved_node-1][change.new_route] + 1;

      if(region_freq[change.day][change.old_route][reg[change.moved_node-1]]==1){
        total_region = total_region 
              + (region_time[reg[change.moved_node-1]][change.old_route]-1)*reg_weight[region_time[reg[change.moved_node-1]][change.old_route]-1]
              - region_time[reg[change.moved_node-1]][change.old_route]*reg_weight[region_time[reg[change.moved_node-1]][change.old_route]];


        region_time[reg[change.moved_node-1]][change.old_route] = region_time[reg[change.moved_node-1]][change.old_route] - 1;
      }

      if(region_freq[change.day][change.new_route][reg[change.moved_node-1]]==0){
        total_region = total_region
              +(region_time[reg[change.moved_node-1]][change.new_route]+1)*reg_weight[region_time[reg[change.moved_node-1]][change.new_route]+1]
              - region_time[reg[change.moved_node-1]][change.new_route]*reg_weight[region_time[reg[change.moved_node-1]][change.new_route]];


        region_time[reg[change.moved_node-1]][change.new_route] = region_time[reg[change.moved_node-1]][change.new_route] + 1;
      }
              

      region_freq[change.day][change.old_route][reg[change.moved_node-1]] = region_freq[change.day][change.old_route][reg[change.moved_node-1]]-1;
      region_freq[change.day][change.new_route][reg[change.moved_node-1]] = region_freq[change.day][change.new_route][reg[change.moved_node-1]]+1;
      ...
      tau[change.moved_node-1][change.day][change.old_route] = lam + tabu_length;
  ...
  flag_writeall = writedataout(routebest, objbest,angle, tabu_length, total_time, travelbest, consistbest, regionbest);
  flag_writerecord = writerecordout(objbest, tabu_length, total_time, travelbest, consistbest, regionbest);
  //print_all_routes(route);
