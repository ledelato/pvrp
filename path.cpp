 ...
 * Initialize a path with zero cost and zero demand
 */
Path::Path(void) {
  cost = 0;
  demand = 0;
...
/**
 * Initialize a path with n nodes, cost c, and demand d
 */
...
bool Path::operator==(Path &p2) {
  ...
  for (it1 = nodes.begin(), it2 = p2.nodes.begin();
      it1 != nodes.end() && it2 != p2.nodes.end(); it1++, it2++)
    if (*it1 != *it2)
      return false;
