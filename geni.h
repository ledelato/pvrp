...
vector<vector<vector<int> > > region_freq(d_total, vector<vector<int> >(v_total, vector<int>(l_total, 0)));			/*how many nodes in the region that a vehicle visits*/
   ...
   double d: distance matrix*/
{
	...
	if (newroute.nodes.empty()) return 0;
...
bool isinp(int node1, int node2,const vector<vector<int> >& nearnb, int p)
{
	...
	for (i=0; i<p; i= i+1)
	{
		...
		if(nearnb[node1][i]==node2) return 1;
	}
	return 0;
}

/*This function determines if route contains q of p-neighborhood of node*/
bool isnear(const list<int>& oldroute, int node, const vector<vector<int> > &neighbor, int p, int q)
{
    //printroute(route);
	...
	for (it = route.begin(); it!=route.end(); ++it)
	{
		...
		if (isinp(node, *it, neighbor, p)&&(*it!=0)) n = n+1;		/*exclude depot*/
	}
	//route.~list();
...
bool isfeasible(const vector<vector<int> >& assign, double cap, double **demand, int d_total, int v_total, int n_total)
...
list<int> remove1(const list<int>& original_route, int j, int k, int oldnode, int*myints)
{
...
list<int> remove2(const list<int>& original_route, int l, int j, int k, int oldnode, int*myints)
{
...
list<int> insert1(const list<int>& original_route, int idi, int idj, int idk, int newnode, int*myints, int size, int category)
{
	int i = -1;
...
list<int> insert2(const list<int>& original_route, int i, int l, int j, int k, int newnode, int *myints, int size, int category)
{
...
Path geni_insert(const Path& original_route, int newnode, int day, int vehicle, int p, double **arr_d, int *myints, int size)
{
...
//	oldcost = tcost(oldroute, new_d);
	...
	else
	{
		...
		for(i = 0; i<p && i<neighbours[day][vehicle][newnode].size(); i++)
		{
			...
			for(j = 0; j<p && j<neighbours[day][vehicle][newnode].size(); j = j+1)
			{
				if (i==j) continue;
				...
				vector <int> tmpv;
				for(k = 0; k<p && k<neighbours[day][vehicle][next[vi]].size() ; k = k+1)
				{
					tmpv.push_back(neighbours[day][vehicle][next[vi]][k]);
				}

				for(k = 0; k<p && k<neighbours[day][vehicle][prev[vi]].size() ; k = k+1)
				{
				...
				for(k = 0; k<2*p && k<tmpv.size() ; k = k+1)
				{
					...
					if (order == 0) {
						tempsave = new_d[vi][newnode] + new_d[newnode][vj] - new_d[vi][next[vi]] - new_d[vj][next[vj]]
									+ new_d[next[vi]][vk] - new_d[vk][next[vk]] + new_d[next[vj]][next[vk]];
					} else {
						tempsave = new_d[vi][newnode] + new_d[newnode][vj] - new_d[vi][prev[vi]] - new_d[vj][prev[vj]]
									+ new_d[prev[vi]][vk] - new_d[vk][prev[vk]] + new_d[prev[vj]][prev[vk]];
					}

					if (DEBUG)
						cout << newnode << " " << vi << " " << vj << " " << vk << " " << tempsave << endl;
		...
		if(flagchange==1)
		{
			...
			{
				cout << "here" << endl;
				cout << newnode << " " << newi << " " << newj << " " << newk << " " << tempsavebest << endl;
				system("pause");
			}
			newroute.nodes = insert1(oldroute.nodes, newi, newj, newk, newnode, myints, size, bestorder);
			if (DEBUG)
				cout << "Cost : " << oldroute.cost  << " " << tempsavebest << " " << tcost(newroute, new_d) << endl;
		...
		if(size>=5)
		{
			for(i = 0; i<p && i<neighbours[day][vehicle][newnode].size() ; i++)
			{
				...
				for(j = 0; j<p && j<neighbours[day][vehicle][newnode].size(); j++)
				{
					int vj = neighbours[day][vehicle][newnode][j];
					...
					set <int> tmps1;
					for (k=0 ; k<p && k<neighbours[day][vehicle][next[vi]].size() ; k++)
						tmps1.insert(neighbours[day][vehicle][next[vi]][k]);
					for (k=0 ; k<p && k<neighbours[day][vehicle][prev[vi]].size() ; k++)
					...
					for (it = tmps1.begin() ; it != tmps1.end() ; it++) 
					{
						...
						set <int> tmps2;
						for (l=0 ; l<p && l<neighbours[day][vehicle][next[vj]].size() ; l++)
							tmps2.insert(neighbours[day][vehicle][next[vj]][l]);
						for (l=0 ; l<p && l<neighbours[day][vehicle][prev[vj]].size() ; l++)
						...
						for (it2 = tmps2.begin() ; it2 != tmps2.end() ; it2++)
						{
							...
							*/
							...
							flagfeasible = 0;
							if(((ii<ll)&&(ll<jj)&&(jj<kk))||((ll<jj)&&(jj<kk)&&(kk<ii))||((jj<kk)&&(kk<ii)&&(ii<ll))||((kk<ii)&&(ii<ll)&&(ll<jj)))
							{
								category = 0;
								flagfeasible = 1;
							...
							} else if(((kk<jj)&&(jj<ll)&&(ll<ii))||((jj<ll)&&(ll<ii)&&(ii<kk))||((ll<ii)&&(ii<kk)&&(kk<jj))||((ii<kk)&&(kk<jj)&&(jj<ll)))
							{
								category = 1;
								flagfeasible = 1;
							...
							if((vk!=vj)&&(vk!=nextj)&&(vl!=vi)&&(vl!=nexti)&&flagfeasible)
							{

								tempsave = new_d[newnode][vj]+new_d[nextj][vl]+new_d[prek][prel]+new_d[vk][nexti]+new_d[vi][newnode]-new_d[nextj][vj]-new_d[vk][prek]-new_d[vi][nexti]-new_d[prel][vl];
			...
			if(flagchange==1)
			{
				temproute.nodes = insert2(oldroute.nodes, newi, newl, newj, newk, newnode, myints, size, newcate);
				if (DEBUG)
					cout << "Cost : " << oldroute.cost + tempsavebest << " " << tcost(temproute, new_d) << endl;
...
Path geni_insert_cached(const Path& original_route, int newnode, int day, int vehicle, int p, double **arr_d, int *myints, int size)
{
	if (insert_cache[newnode][vehicle][day] == NULL ) 
	{
		Path * path = new Path();
		*path = geni_insert(original_route, newnode, day, vehicle, p, arr_d, myints, size);
...
Path geni_remove(const Path& original_route, int newnode, int day, int p, double **arr_d, int *myints, int size)
{
	double start = clock();
	double tempbest = DBL_MAX;
	double tempcost;
	...
	else
	{
		for(j = 0; j<size-2; j = j+1)
		{
			  ...
			  {	
				  ...
				  tempsave = new_d[idi][idj]+new_d[nexti][idk]+new_d[nextj][nextk]-new_d[idi][newnode]-new_d[newnode][nexti]-new_d[idj][nextj]-new_d[idk][nextk];
		...
		if (DEBUG)
			cout << "Costs : " << oldroute.cost + tempsavebest << " " << tcost(newroute, new_d) << endl;
		...
		if(size>4)
		{
		  ...
		  for(l = 0; l<size-2; l = l+1)
		  {
			  ...
			  {		
				  for(k = j+1; k<size-2; k = k+1)
				  {
					  ...
					  tempsave = new_d[nexti][nextl]+new_d[idj][nextk]+new_d[idi][idk]+new_d[nextj][idl]-new_d[newnode][nexti]-new_d[idl][nextl]-new_d[idj][nextj]-new_d[idk][nextk]-new_d[idi][newnode];
		  ...
		  temproute.nodes = remove2(oldroute.nodes, newl, newj, newk, newnode, myints);
		  if (DEBUG)
			cout << "Costs : " << oldroute.cost + tempsavebest << " " << tcost(temproute, new_d) << endl;
		  //assert(abs(oldroute.cost + tempsavebest - tcost(temproute, new_d)) < EP);
		  temproute.cost = oldroute.cost + tempsavebest;		  
		  temproute.demand = oldroute.demand - de_rank[newnode-1][day];
...
Path geni_remove_cached(int newnode, int routeId, int day, const Path& original_route, int p, double **arr_d, int *myints, int size)
{
